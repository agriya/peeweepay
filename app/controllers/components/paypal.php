...
class PaypalComponent extends Object
{
    ...
    public function sanitizeServerVars()
    {
        ...
        $expected_paypal_post_arr = array(
            ...
            'reverse_all_parallel_payments_on_error' => 0,
            'pay_key' => 0,
            'memo' => '',
            'reason_code' => '',
            'tracking_id' => '',
        ...
        if (!empty($_POST['transaction'])) {
            $transaction_count = count($_POST['transaction']);
            $transaction_array = array();
            $i = 0;
            foreach($_POST['transaction'] as $key => $default_value) {
                $transaction_array['transaction'][$i][$key] = htmlspecialchars(trim($default_value));
                $i++;
            }
            $tmp_arr = array_merge($tmp_arr, $transaction_array);
        }
    ...
    public function logPaypalTransactions()
    {
        ...
        $paypalTransactionModel = ClassRegistry::init('AdaptiveTransactionLog');
        $log['AdaptiveTransactionLog']['paypal_response'] = $this->paypal_response;
        $log['AdaptiveTransactionLog']['error_no'] = $this->errno;
        $log['AdaptiveTransactionLog']['error_message'] = $errorMessaegString;
        $log['AdaptiveTransactionLog']['paypal_post_vars'] = $this->paypal_post_vars_in_str;
        $log['PaypalTransactionLog']['ip'] = $this->RequestHandler->getClientIP();
        $paypalTransactionModel->save($log);
    ...
    function nvpHeader($sender_info)
    {
     ...
     */
    function doDirectPayment($post_info, $sender_info)
    {
        /**
     ...
     * returns an associtive array containing the response from the server.
     */
    function doCapture($post_info, $sender_info)
    {
        /**
         * Get required parameters from the web form for the request
         */
     ...
     * returns an associtive array containing the response from the server.
     */
    function doVoid($post_info, $sender_info)
    {
        /**
         * Get required parameters from the web form for the request
         */
     ...
     * returns an associtive array containing the response from the server.
     */
    function hash_call($methodName, $nvpStr, $sender_info)
    {
        // Set request-specific fields.
        $API_Endpoint = ($sender_info['is_testmode']) ? $this->do_direct_pay_constants['paypal_url']['test_mode'] : $this->do_direct_pay_constants['paypal_url']['live_mode'];
        // Set up your API credentials, PayPal end point, and API version.
