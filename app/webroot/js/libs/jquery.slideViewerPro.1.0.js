...
jQuery.fn.slideViewerPro = function(settings) {
	...
	return this.each(function(){
		...
		var r_enabled = true;
		var l_enabled = true;
    container.find("ol").css("width" , slideViewerWidth)
    .wrap(jQuery("<div style='width:"+ pictWidth +"px; overflow: hidden; position: relative; top: 0; left: 0'>"));
		...
		container.each(function(i) {
			...
			jQuery(this).after("<div class='thumbSlider' id='thumbSlider" + j + "'><ol><\/ol><\/div>");
			...
			jQuery(this).find("li").each(function(n) { 
						jQuery("div#thumbSlider" + j + " ol").append("<li><a title='" + jQuery(this).find("img").attr("title") + "' href='#'><img width='"+ thumbsWidth +"' height='"+ thumbsHeight +"' src='" + jQuery(this).find("img").attr("src") + "' /><p class='tmbrdr'>&nbsp;<\/p><\/a><\/li>");						
			});
			...
			jQuery("div#thumbSlider" + j + " a").each(function(z) {			
				jQuery(this).bind("click", function(){
					...
					var cnt = -(pictWidth*z);
					(cnt != container.find("ol").css("left").replace(/px/, "")) ? container.find("span.typo").animate({"opacity": 0}, 250) : null ;
					container.find("ol").animate({ left: cnt}, settings.easeTime, settings.easeFunc, function(){container.find("span.typo").animate({"opacity": settings.typoFullOpacity}, 250)});					
					return false;
				});
			});
			...
			jQuerybtr.bind("click", function(){
					...
					r_enabled = false;
					jQuery(this).prev().prev().find("ol:not(:animated)").animate({ left: -(thumbsWidth+settings.thumbsRightMargin)*pos}, 500, settings.easeFunc, function(){authorityMixing();});					
					return false;
			});
			jQuerybtl.bind("click", function(){	
					...
					l_enabled = false;
					jQuery(this).prev().find("ol:not(:animated)").animate({ left: -(thumbsWidth+settings.thumbsRightMargin)*pos}, 500, settings.easeFunc, function(){authorityMixing();});			
					return false;
			});						
			...
			jQuery("div#thumbSlider" + j + " ol").css({width: (thumbsWidth*pictEls)+settings.thumbsRightMargin*pictEls, position: "relative", left: "0", top: "0"});
			jQuery("div#thumbSlider" + j + " ol li").css({marginRight: settings.thumbsRightMargin});
			...
			if(settings.autoslide){
					...
					jQuery("div#thumbSlider" + j).everyTime(settings.asTimer, "asld", function() {			
		  			...
		  			if(i == 0)
		  			{
						pos = 0;
						l_enabled = false;
						jQuery("div#thumbSlider" + j).find("ol:not(:animated)").animate({ left: -(thumbsWidth+settings.thumbsRightMargin)*pos}, 500, settings.easeFunc, function(){authorityMixing();});
		  			}
		  			else l_enabled = true;
