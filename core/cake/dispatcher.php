<?php
/* SVN FILE: $Id: dispatcher.php 241 2009-05-30 14:45:16Z rajesh_04ag02 $ */
/**
 ...
 * @lastmodified  $Date: 2009-02-18 05:02:15 +0530 (Wed, 18 Feb 2009) $
...
// another fix in router.php#851
        $t_action = $this->params['action'];
        if (!empty($this->params['prefix'])) {
			$t_action = substr($this->params['action'], strlen($this->params['prefix'] . '_'));
        }
...
//Note (by abuthakir_47ag05): Added url in the params array to fix the problem in redirect when calling through ajax
$this->params['url']['url'] = Router::url($url);
$parsed_url_arr = Router::parse($this->params['url']['url']);
if (!empty($parsed_url_arr) and !empty($parsed_url_arr['named'])) {
	$this->params['named'] = $parsed_url_arr['named'];
}
if (isset($url[0])){
    $this->params['pass'][] = $url[0];
}
// Additional fix by Siva for admin routing issue; was always throwing private action error when calling from ajax
// @todo: Now, the $this->params['action'] on ajax gets just action. e.g., 'index' instead of the correct 'admin_index'
$privateAction = (bool)(strpos($this->params['action'], '_', 0) === 0);
$prefixes = Router::prefixes();
...
// Siva // Fix for reverse routing namedArg is set in param wrongly
		...
		$keys = array_values(array_diff(array_keys($this->params), $skip));
		$count = count($keys);
		for ($i = 0; $i < $count; $i++) {
			if ($i === 0 && is_numeric($keys[$i]) && in_array('id', $keys)) {
				$this->params['pass']['0'] = $this->params[$keys[$i]];
			} elseif (is_numeric($keys[$i]) || $keys[$i] === 'id') {
				$this->params['pass'][] = $this->params[$keys[$i]];
			} else {
				$this->params['named'][$keys[$i]] = $this->params[$keys[$i]];
			}
...
// Siva // Sanitizing for SQL injection issue
                if (!in_array($params['form']['_method'], array(
                    'post',
                    'put',
                    'get',
                    'delete'
                ))) {
                    $params['form']['_method'] = 'post';
                }
// <--
...
// Siva // Fix for reverse routing namedArg is set in param wrongly
		...
		for ($i = 0; $i < $count; $i++) {
			if ($i === 0 && is_numeric($keys[$i]) && in_array('id', $keys)) {
				$params['pass']['0'] = $params[$keys[$i]];
			} elseif (is_numeric($keys[$i]) || $keys[$i] === 'id') {
				$params['pass'][] = $params[$keys[$i]];
			} else {
				$params['named'][$keys[$i]] = $params[$keys[$i]];
			}
			unset($params[$keys[$i]]);
		}
// <--
