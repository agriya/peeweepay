<?php
/* SVN FILE: $Id: shell.php 181 2009-02-10 11:21:09Z rajesh_04ag02 $ */
/**
 ...
 */
	function createFile ($path, $contents) {
		...
		if (is_file($path) && $this->interactive === true) {
			$key = $this->in(__("File exists, overwrite?", true). " {$path}",  array('y', 'n', 'q'), 'y');
			if (low($key) == 'q') {
 ...
 */
	function _checkUnitTest() {
		...
		$unitTest = $this->in('Cake test suite not installed.  Do you want to bake unit test files anyway?', array('y','n'), 'n');
...
//rajesh_04ag02 // 2008-07-02
    function beautifyPHP($code) {
        ...
        // 1st pass.. needed to handle lowercase; because of bug in lib
        $oBeautifier1 = new PHP_Beautifier();
        $oBeautifier1->addFilter('Lowercase');
        $oBeautifier1->setInputString($code);
        $oBeautifier1->process();
        $oBeautifier2 = new PHP_Beautifier();
        // 2nd pass
        $oBeautifier2->addFilter('Pear');
        $oBeautifier2->addFilter('ArrayNested');
        $oBeautifier2->setIndentChar(' ');
        $oBeautifier2->setIndentNumber(4);
        $oBeautifier2->setNewLine("\n");
        $oBeautifier2->setInputString($oBeautifier1->get());
        unset($oBeautifier1);
        $oBeautifier2->process();
        return $oBeautifier2->get();
    }
}
?>
