<?php
/* SVN FILE: $Id: controller.php 173 2009-01-31 12:51:40Z rajesh_04ag02 $ */
/**
 ...
 */
	function bakeActions($controllerName, $admin = null, $wannaUseSession = true) {
		...
		$singularHumanName = Inflector::humanize(Inflector::underscore($currentModelName));
		$pluralHumanName = Inflector::humanize(Inflector::underscore($controllerName));
		$actions .= "\n";
		/* index action */
		$actions .= "\tfunction {$admin}index() {\n";
		$actions .= "\t\t\$this->pageTitle = __l('$pluralHumanName');\n";
		$actions .= "\t\t\$this->{$currentModelName}->recursive = 0;\n";
		$actions .= "\t\t\$this->set('{$pluralName}', \$this->paginate());\n";
		$actions .= "\t}\n";
		$actions .= "\n";
		/* view action */
		$_fieldNames = array_keys($modelObj->_schema);
        $_slugName = 'id';
		...
		$actions .= "\tfunction {$admin}view(\${$_slugName} = null) {\n";
		$actions .= "\t\t\$this->pageTitle = __l('$singularHumanName');\n";
		$actions .= "\t\tif (is_null(\${$_slugName})) {\n";
		$actions .= "\t\t\t\$this->cakeError('error404');\n";
		$actions .= "\t\t}\n";
		$actions .= "\t\t\${$singularName} = \$this->{$currentModelName}->find('first', array(\n";
		$actions .= "\t\t\t'conditions' => array(\n";
		$actions .= "\t\t\t\t'{$currentModelName}.{$_slugName} = ' => \${$_slugName}\n";
		$actions .= "\t\t\t),\n";
		$actions .= "\t\t\t'fields' => array(\n";
		// fields to fetch
		foreach($_fieldNames as $_fieldName){
    		$actions .= "\t\t\t\t'{$currentModelName}.{$_fieldName}',\n";
        }
        // recursive need to to be increased when showing other model fields in view
        $_recursive = -1;
		foreach ($modelObj->hasAndBelongsToMany as $associationName => $relation) {
			if (!empty($associationName)) {
                $_recursive = 0;
				$habtmModelName = $this->_modelName($associationName);
        		$habtmModelObj =& new $habtmModelName();
        		$_fieldNames = array_keys($habtmModelObj->_schema);
        		foreach($_fieldNames as $_fieldName){
            		$actions .= "\t\t\t\t'{$habtmModelName}.{$_fieldName}',\n";
                }
			}
		}
		foreach ($modelObj->belongsTo as $associationName => $relation) {
			if (!empty($associationName)) {
        		...
        		$belongsToModelObj =& new $belongsToModelName();
        		$_fieldNames = array_keys($belongsToModelObj->_schema);
        		foreach($_fieldNames as $_fieldName){
            		$actions .= "\t\t\t\t'{$belongsToModelName}.{$_fieldName}',\n";
                }
			}
		}
		$actions .= "\t\t\t),\n";
		$actions .= "\t\t\t'recursive' => ". $_recursive .",\n";
		$actions .= "\t\t));\n";
		$actions .= "\t\tif (empty(\${$singularName})) {\n";
		$actions .= "\t\t\t\$this->cakeError('error404');\n";
		$actions .= "\t\t}\n";
		$actions .= "\t\t\$this->pageTitle .= ' - ' . \${$singularName}['{$currentModelName}']['{$modelObj->displayField}'];\n";
		...
		$actions .= "\t\t\$this->pageTitle = __l('Add $singularHumanName');\n";
		...
		$actions .= "\t\t\$this->pageTitle = __l('Edit $singularHumanName');\n";
		$actions .= "\t\tif (is_null(\$id)) {\n";
		$actions .= "\t\t\t\$this->cakeError('error404');\n";
		$actions .= "\t\t}\n";
		$actions .= "\t\tif (!empty(\$this->data)) {\n";
		$actions .= "\t\t\tif (\$this->{$currentModelName}->save(\$this->data)) {\n";
...
//			$actions .= "\t\t\t\t\$this->redirect(array('action' => 'index'));\n";
		...
		$actions .= "\t\t\t}\n";
		$actions .= "\t\t}\n";
		$actions .= "\t\t\$this->pageTitle .= ' - ' . \$this->data['{$currentModelName}']['{$modelObj->displayField}'];\n";
		foreach ($modelObj->hasAndBelongsToMany as $associationName => $relation) {
			if (!empty($associationName)) {
				$habtmModelName = $this->_modelName($associationName);
				$habtmSingularName = $this->_singularName($associationName);
