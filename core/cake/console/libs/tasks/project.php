<?php
/* SVN FILE: $Id: project.php 173 2009-01-31 12:51:40Z rajesh_04ag02 $ */
/**
...
/*
// rajesh_04ag02 // 2008-06-04 // Commented out the hard coded fix of core path
...
// rajesh_04ag02 // 2008-07-09 // set project name
    function _setProjectName($path)
    {
        $_projectName = basename(dirname($path));
        $File =& new File(CONFIGS . 'config.php');
		$contents = $File->read();
		$result = str_replace('dev1framework', $_projectName, $contents);
		if ($File->write($result)) {
	   	   $this->out('config.php set with project name-' . $_projectName);
		} else {
    		$this->out('config.php could not be set with project name-' . $_projectName);
		}
        $File =& new File(CONFIGS . 'database.php');
		$contents = $File->read();
		$result = str_replace('dev1framework', $_projectName, $contents);
		if ($File->write($result)) {
	   	   $this->out('database.php set with project name-' . $_projectName);
		} else {
    		$this->out('database.php could not be set with project name-' . $_projectName);
		}
        $File =& new File(APP . '.htaccess');
		$contents = $File->read();
		$result = str_replace('dev1framework', $_projectName, $contents);
		if ($File->write($result)) {
	   	   $this->out('APP .htaccess set with project name-' . $_projectName);
		} else {
    		$this->out('APP .htaccess could not be set with project name-' . $_projectName);
		}
        $File =& new File(APP . 'webroot' . DS . '.htaccess');
		$contents = $File->read();
		$result = str_replace('dev1framework', $_projectName, $contents);
		if ($File->write($result)) {
	   	   $this->out('webroot .htaccess set with project name-' . $_projectName);
		} else {
    		$this->out('webroot .htaccess could not be set with project name-' . $_projectName);
		}
    }
}
?>
