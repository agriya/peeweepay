<?php
/* SVN FILE: $Id: extract.php 252 2009-08-26 15:55:44Z rajesh_059at09 $ */
/**
 ...
 * @lastmodified  $Date: 2009-02-13 11:52:34 +0530 (Fri, 13 Feb 2009) $
	...
	function __buildFiles() {
		foreach ($this->__strings as $str => $fileInfo) {
			...
			if ($this->__oneFile === true) {
				...
				$langs_sql = '';
				foreach($occured AS $langs_occured) {
					$langs_sql .= "\n" . 'INSERT INTO translations(`created`, `modified`, `language_id`, `key`, `lang_text`) VALUES (NOW(), NOW(), "38", "'.addslashes($str).'", "'.addslashes($str).'" );';
				}
				$occurances = join("\n#: ", $occured);
