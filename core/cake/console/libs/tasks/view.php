<?php
/* SVN FILE: $Id: view.php 173 2009-01-31 12:51:40Z rajesh_04ag02 $ */
/**
 ...
 */
	function __loadController() {
		...
		if ($modelObj) {
			...
			$pluralHumanName = Inflector::humanize(Inflector::underscore($this->controllerName));
			...
			$pluralVar = Inflector::variable($this->controllerName);
			$singularHumanName = Inflector::humanize(Inflector::underscore(Inflector::singularize($this->controllerName)));
			$pluralHumanName = Inflector::humanize(Inflector::underscore($this->controllerName));
			$fields = array();
			$schema = array();
			$associations = array();
...
// rajesh_04ag02 // 2008-06-30
		if (in_array($template, array('index'))) {
    		$_isIndexList = $this->in(('****=== Would you like to have list (OL) based index view?'."\n".'****=== Choosing "no" will create table based listing.'), array('y','n'), 'y');
    		if ((low($_isIndexList) == 'y' || low($_isIndexList) == 'yes')) {
    			$template = 'index_list';
    		}
		}
...
// rajesh_04ag02 // 2008-07-07 // Was just array_keys()
				$associations[$type][$assocKey]['fields'] =  ($model->{$assocKey}->schema());
			}
		}
		return $associations;
	}
