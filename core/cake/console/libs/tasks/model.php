<?php
/* SVN FILE: $Id: model.php 251 2009-08-26 15:55:10Z rajesh_059at09 $ */
/**
 ...
 * @lastmodified  $Date: 2008-12-04 01:54:24 +0530 (Thu, 04 Dec 2008) $
...
// rajesh_04ag02 // 2008-06-30
			foreach ($fields as $name => $field) {
				if (in_array($name, array('username', 'name', 'title', 'display_name'))) {
                    $_displayField = $name;
                    break;
				}
			}
			$_displayField = $this->in(__('What is the displayField?', true), null, $_displayField);
            $_isSlug = (in_array('slug', array_keys($fields)));
            $_isOrder = (in_array('display_order', array_keys($fields)));
		}
		...
		if (low($looksGood) == 'y' || low($looksGood) == 'yes') {
			if ($this->bake($currentModelName, $associations, $validate, $primaryKey, $useTable, $useDbConfig, $_displayField, $_isSlug, $_isOrder)) {
				if ($this->_checkUnitTest()) {
...
// Partial changes for allowEmpty addition
            $allowEmpty = 'true';
			...
			if ($choice != $skip) {
				if (is_numeric($choice) && isset($choices[$choice])) {
					$validate[$fieldName]['rule'] = $choices[$choice];
				} else {
					$validate[$fieldName]['rule'] = $choice;
				}
                $choiceAllowEmpty = $this->in(__('Is allow empty?', true), array('y','n'), 'n');
    			$validate[$fieldName]['allowEmpty'] = $allowEmpty;
                if ((low($choiceAllowEmpty) == 'n' || low($choiceAllowEmpty) == 'no')) {
    			     $validate[$fieldName]['allowEmpty'] = 'false';
    		    }
    		    $validate[$fieldName]['message'] = $this->in(__('Enter validation message', true), null, 'Required');
            }
// <--
		}
		return $validate;
...
// Fix from Siva (not all codes) -->
		...
		foreach ($fields as $fieldName => $field) {
			...
			if ($fieldName != $model->primaryKey && $offset !== false) {
				...
				$tempOtherModel = & new Model(array('name' => $tmpModelName));
                $relatedModelfields = $tempOtherModel->schema();
                if(!empty($relatedModelfields)){
                    foreach($relatedModelfields as $relatedModelfield => $value){
                        if($relatedModelfield == Inflector::singularize($model->table) . '_count'){
                            $associations['belongsTo'][$i]['counterCache'] = 'true';
                            continue;
                        }
                    }
                }
 ...
 * @param string $_displayField displayField // rajesh_04g02 // 2008-06-30
 * @param string $_isSlug Whether to add Sluggable behavior // rajesh_04ag02 // 2008-07-03
 * @param string _isOrder Whether to add $order and List Behavior // rjaesh_04ag02 // 2008-07-10
 * @access private
 */
	function bake($name, $associations = array(),  $validate = array(), $primaryKey = 'id', $useTable = null, $useDbConfig = 'default', $_displayField, $_isSlug, $_isOrder) {
		...
		if (!empty($_displayField)) {
			$out .= "\tvar \$displayField = '$_displayField';\n";
		}
        if ($_isOrder) {
// commented out as it's producing inefficient table scan
//			$out .= "\tvar \$order = array('{$name}.display_order');\n";
        }
		$_actsAs = '';
		if ($_isSlug) {
            $_actsAs .= "\t\t'Sluggable' => array(\n";
            $_actsAs .= "\t\t\t'label' => array(\n";
            $_actsAs .= "\t\t\t\t'".$_displayField."'\n";
            $_actsAs .= "\t\t\t)\n";
            $_actsAs .= "\t\t),\n";
        }
        if ($_isOrder) {
...
// for translation -->
		...
		if (is_array($validate) && $validateCount > 0) {
            $out .= "\t//\$validate set in __construct for multi-language support\n";
			$_vout .= "\t\t\$this->validate = array(\n";
			$keys = array_keys($validate);
			for ($i = 0; $i < $validateCount; $i++) {
				$_vout .= "\t\t\t'" . $keys[$i] . "' => array('rule' => '" . $validate[$keys[$i]]['rule']."'";
				if (!empty($validate[$keys[$i]]['allowEmpty'])){
                    $_vout .= ", \n\t\t\t 'allowEmpty' => " . $validate[$keys[$i]]['allowEmpty'];
                }
  				if (!empty($validate[$keys[$i]]['message'])){
                    $_vout .= ", \n\t\t\t  'message' => __l('" . $validate[$keys[$i]]['message']."')";
                }
                $_vout .= "\n\t\t)";
		...
		if (!empty($associations)) {
			...
			if (!empty($associations['belongsTo'])) {
				...
				for ($i = 0; $i < $belongsToCount; $i++) {
					...
					$out .= "\t\t\t\t\t\t\t\t'conditions' => '',\n";
					$out .= "\t\t\t\t\t\t\t\t'fields' => '',\n";
// From Siva for counterCache
					$out .= "\t\t\t\t\t\t\t\t'order' => '',\n";
					if (isset($associations['belongsTo'][$i]['counterCache'])) {
						$out .= "\t\t\t\t\t\t\t\t'counterCache' => {$associations['belongsTo'][$i]['counterCache']}\n";
					}
// <--
                    $out .= "\t\t\t)";
...
// for translation -->
        $out .= "\n";
		$out .= "\t".'function __construct($id = false, $table = null, $ds = null) {'."\n";
        $out .= "\t\t".'parent::__construct($id, $table, $ds);'."\n";
		$out .= $_vout;
        $out .= "\t}\n";
        $out .= "}\n";
// <-- for translation
