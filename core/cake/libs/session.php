<?php
/* SVN FILE: $Id: session.php 227 2009-05-26 15:10:51Z rajesh_04ag02 $ */
/**
 ...
 * @lastmodified  $Date: 2009-01-14 09:22:37 +0530 (Wed, 14 Jan 2009) $
...
// Siva // user_id into session table
		...
		if ($row[0][0]['count'] > 0) {
			$db->execute("UPDATE " . $db->name($table) . " SET " . $db->name('data') . " = "
								...
								. $db->value($expires) . ", " . $db->name('user_id') . " = "
								. $db->value($user_id) . " WHERE " . $db->name('id') . " = "
								. $db->value($key));
		} else {
			$db->execute("INSERT INTO " . $db->name($table) . " (" . $db->name('data') . ","
							  	. $db->name('expires') . "," . $db->name('id') . "," . $db->name('user_id')
							  	. ") VALUES (" . $db->value($value) . ", " . $db->value($expires) . ", "
							  	. $db->value($key) . ", " . $db->value($user_id) . ")");
// <--
		}
		return true;
	}
