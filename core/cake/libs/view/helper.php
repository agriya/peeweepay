<?php
/* SVN FILE: $Id: helper.php 275 2010-03-02 15:21:00Z rajesh_059at09 $ */
/**
 * Backend for helpers.
 *
 ...
 * @lastmodified  $Date: 2009-01-27 01:15:55 +0530 (Tue, 27 Jan 2009) $
 ...
 */
    ...
    {
//Was	return Router::url($url, array('full' => $full, 'escape' => true));
// Siva // for caching
        $keyUrl = $url;
        if (is_array($keyUrl)) {
            $keyUrl+= $this->_extras;
        ...
        $this->_cache[$key] = Router::url($url, array(
            'full' => $full,
            'escape' => true
        ));
        return $this->_cache[$key];
...
// @todo More cleanup of changes
    ...
    {
        parent::__construct();
        if (!isset($this->htmlPurifier)) { // fix for memory exhaust
            ...
            App::import('Vendor', 'Numbers_Words', true, array() , 'PEAR' . DS . 'Numbers' . DS . 'Words.php');
	...
	function cCurrency($str, $cur_code = '', $wrap = 'span', $title = false)
    {
        $_precision = 2;
        $changed = (($r = floatval($str)) != $str);
        $rounded = (($rt = round($r, $_precision)) != $r);
        ...
        if ($wrap) {
            if (!$title) {
                $title = Numbers_Words::toCurrency($r, 'en_US', $cur_code);
            }
            $r = '<' . $wrap . ' class="c' . $changed . ' cr' . $rounded . '" title="' . $title . '">' . number_format($r, $_precision, '.', ',') . '</' . $wrap . '>';
        }
        return $r;
    ...
    {
        $changed = (($r = $this->htmlPurifier->purify(date(Configure::read('site.date.format') , strtotime($str)))) != date(Configure::read('site.date.format') , strtotime($str)));
        if ($wrap) {
            if (!$title) {
                //$title = date('F j, Y g:i:s A (l) T (\U\TC Z \G\M\TP)', strtotime($str));
    ...
    {
        $changed = (($r = $this->htmlPurifier->purify(date(Configure::read('site.datetime.format') , strtotime($str)))) != date(Configure::read('site.datetime.format') , strtotime($str)));
        if ($wrap) {
            if (!$title) {
                //$title = date('F j, Y g:i:s A (l) T (\U\TC Z \G\M\TP)', strtotime($str));
    ...
    {
        $changed = (($r = $this->htmlPurifier->purify(date(Configure::read('site.time.format') , strtotime($str)))) != date(Configure::read('site.time.format') , strtotime($str)));
        if ($wrap) {
            if (!$title) {
    ...
    {
        return $this->Text->truncate($text, $length, $ending, $exact, $considerHtml);
    }
