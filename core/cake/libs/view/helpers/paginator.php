<?php
/* SVN FILE: $Id: paginator.php 249 2009-07-16 17:07:10Z rajesh_04ag02 $ */
/**
 ...
 * @lastmodified  $Date: 2009-02-04 00:18:28 +0530 (Wed, 04 Feb 2009) $
...
// rajesh_04ag02 // 2009-07-16 // retain params
	function __construct(){
        parent::__construct();
        // Kinda hack as $this->params is empty here
        $view = & ClassRegistry::getObject('view');
        $this->options(array(
            'url' => array_merge(array(
                'controller' => $view->params['controller'],
                'action' => $view->params['action'],
            ) , $view->params['pass'], $view->params['named'])
        ));
    }
// <--
/**
...
// rajesh_04ag02 // sort header class
        if ($this->sortKey($options['model']) == $key && $this->sortDir($options['model']) == 'desc') {
            if (empty($options['class'])) {
                $options['class'] = 'desc';
            } else {
                $options['class'] = 'desc ' . $options['class'];
            }
        }
// <--
...
// rajesh_04ag02 // sort header class
            if (empty($options['class'])) {
                ...
                $options['class'] = 'asc ' . $options['class'];
            }
// <--
		}
	...
	function numbers($options = array()) {
		...
		$options = array_merge(
			array(
				...
				'skip' => ' ... ', //// @author rajesh_04ag02 // 2008-05-19
			),
		(array)$options);
		...
		unset($options['tag'], $options['before'], $options['after'], $options['model'],
			$options['modulus'], $options['separator'], $options['first'], $options['last'],
              $options['skip'] // @author rajesh_04ag02 // 2008-05-19
        );

		$out = '';

        $half = $end = PHP_INT_MAX;
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			$out .= $before;
//-->
// Taken from http://bin.cakephp.org/view/656125640 for having skip option
// Done with minor changes
// @author rajesh_04ag02 // 2008-05-19
        if ($start!=1 && $start!=2) {
            $floor = 0;
            if ($params['page']-$half > 1) {
                $out.= $separator . '<span>' . $this->link('1', array(
                    'page' => '1'
                ) , $options) . '</span>';
                $floor = 1;
            }
            if ($params['page']-$half > 2) {
                $out.= $separator . '<span>' . $this->link('2', array(
                    'page' => '2'
                ) , $options) . '</span>';
                $floor = 2;
            }
            if ($params['page']-($half) > $floor+1) {
                $out.= $skip . $separator;
            }
        }
//<--
...
// @author rajesh_04ag02 // 2008-05-19
        ...
        if ($end < $params['pageCount']) {
            if ($params['page']+($half+1) < $params['pageCount']-1) {
                $out.= $skip . $separator;
            }
            if ($params['pageCount']-1 != $end) {
                $out.= '<span>' . $this->link($params['pageCount']-1, array(
                    'page' => $params['pageCount']-1
                ) , $options) . '</span>' . $separator;
            }
            $out.= '<span>' . $this->link($params['pageCount'], array(
