<?php
/* SVN FILE: $Id: form.php 263 2009-08-26 16:06:38Z rajesh_059at09 $ */
/**
 ...
 * @lastmodified  $Date: 2009-02-18 07:09:01 +0530 (Wed, 18 Feb 2009) $
...
// Siva // 2009-05-26
// Was		$text = __(Inflector::humanize(Inflector::underscore($text)), true);
			$text = __l(Inflector::humanize(Inflector::underscore($text)));
// <--
		}
...
// Siva: info & help
		$defaults = array('before' => null, 'between' => null, 'after' => null, 'info' => null, 'help' => null);
		$options = array_merge($defaults, $options);
...
// fixed by Siva => required check for foreign model in a form
			$addRequired = false;
            $modelObject = & ClassRegistry::getObject($this->model());
            if (isset($modelObject->validate[$this->field() ])) {
                foreach($modelObject->validate[$this->field() ] as $foreignKey => $foreignValue) {
                    if (!(isset($foreignValue['allowEmpty']) && $foreignValue['allowEmpty'])) {
                        $addRequired = true;
                    }
                    if ($foreignKey == 'allowEmpty' && $foreignValue) {
                        $addRequired = false;
                    }
                }
            }
            if ($addRequired) {
// Was		if (in_array($this->field(), $this->fieldset['validates'])) {
// <--
...
// Siva: info & help
		$info	 = $options['info'];
		$help	 = $options['help'];
		unset($options['type'], $options['before'], $options['between'], $options['after'], $options['info'], $options['help']);
// <--
		switch ($type) {
			case 'hidden':
...
// Siva: info & help
            if (!empty($info)){
                $out .= $this->Html->tag('span', $info, 'info');
            }
            if (!empty($help)){
                $out .= $this->Html->tag('span', $help, 'help');
            }
// <--
			$out .= $after;
			if ($error !== false) {
 ...
 * @return string
 */
// Siva // 2009-01-07 // orderYear option for sorting the Year in desc
	function year($fieldName, $minYear = null, $maxYear = null, $selected = null, $attributes = array(), $showEmpty = true, $orderYear = null) {
		if ((empty($selected) || $selected === true) && $value = $this->value($fieldName)) {
			if (is_array($value)) {
				extract($value);
				$selected = $year;
		...
		$yearOptions = array('min' => $minYear, 'max' => $maxYear, 'order' => $orderYear);
		return $this->select(
 ...
 */
	function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $selected = null, $attributes = array(), $showEmpty = true) {
		...
		$defaults = array(
			...
			'interval' => 1, 'monthNames' => true, 'orderYear' => null
		);
		$attributes = array_merge($defaults, (array) $attributes);
		...
		$orderYear = $attributes['orderYear'];
		...
		if ($dateFormat != 'NONE') {
			...
			foreach (preg_split('//', $dateFormat, -1, PREG_SPLIT_NO_EMPTY) as $char) {
				switch ($char) {
					case 'Y':
						$selects[] = $this->year(
							$fieldName, $minYear, $maxYear, $year, $selectYearAttr, $showEmpty, $orderYear
						);
					break;
					case 'M':
